      ...
      <Fragment>
        <BlockControls>
          <AlignmentToolbar
            value={alignment}
            onChange={(nextAlign) =>
              setAttributes({ alignment: nextAlign || "center" })
            }
          />
        </BlockControls>
        ...
        <InspectorControls>
          <PanelBody title="Button Settings" initialOpen={true}>
            ...
            <TextareaControl
              label="SVG Icon (paste raw SVG)"
              value={svgIcon}
              onChange={(value) => setAttributes({ svgIcon: value })}
              help="Paste full SVG markup. It will be rendered exactly as provided."
            />
